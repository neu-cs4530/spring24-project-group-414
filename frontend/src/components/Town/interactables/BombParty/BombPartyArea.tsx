import { Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, Button, Container, Divider, GridItem, HStack, List, ListItem, Select, SimpleGrid, useToast, VStack } from "@chakra-ui/react";
...
import BombPartyBoard from "./BombPartyBoard";
   ...
   * @returns
   */
  function BombPartyArea({ interactableID }: { interactableID: InteractableID }): JSX.Element {
    ...
    return (
      ...
      <Container minW='full' paddingX='0px'>
        ...
        <Container
          ...
          {status !== 'WAITING_TO_START' && <BombPartyBoard gameAreaController={gameAreaController} />}
        </Container>
      </Container>
    );
  }

export default function BombPartyAreaWrapper(): JSX.Element {
    const gameArea = useInteractable<GameArea>('gameArea');
    const townController = useTownController();
    ...
    if (gameArea && gameArea.getData('type') === 'BombParty') {
      return (
        <Modal size='4xl' isOpen={true} onClose={closeModal} closeOnOverlayClick={false}>
          ...
          <ModalContent>
            ...
            <ModalCloseButton />
            <BombPartyArea interactableID={gameArea.name} />
          </ModalContent>
        </Modal>
      );
    }
